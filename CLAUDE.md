...
- ⚠️ **Paragraph spacing is a contract with Duda, and as of 2026-09-08 the agreed value is ZERO.** Duda styles `<p>` flush by default, so `RichTextEditor.tsx` and `RichHtml.tsx` both use `[&_p]:my-0` to match. **Change one, change all three** (editor, preview, Duda) or the editor stops being trustworthy.

  History worth knowing, because it explains why this flipped twice: the gap was originally added on the OLD site (`099434f3`) as CSS in Duda's **Head HTML**, scoped to a wrapper class Josh added — `.productDescription p:not(:last-child) { margin-bottom: 16px }` — because Duda's theme API exposes **no** margin/spacing property (checked: `paragraph` only takes font/colour/letter-spacing). That is per-site config and did **not** survive the move to `8a8f03b5`: fetching the live page shows neither `.productDescription` nor `margin-bottom:16px` anywhere. So paragraphs render flush there, and rather than re-add the Duda-side CSS, Josh chose (2026-09-08) to make the Hub match the flush rendering.

  **Consequence, accepted deliberately**: with no margin a paragraph break is visually identical to a line break, so staff cannot distinguish them while editing. To restore a gap, add the CSS on Duda's side FIRST (Head HTML + the wrapper class on the description element), then update both components in the same edit. Use an absolute px value, never `1em` — em resolves to 18px on Duda vs 16px in the dashboard, i.e. guaranteed drift.

  Note the sanitiser (`services/descriptionHtml.ts`) still converts a sentence-boundary `<br>` into a paragraph break and drops empty paragraphs. Both are now cosmetically neutral on the live page but keep the stored HTML clean and meaningful, so they stay.

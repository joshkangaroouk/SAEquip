...
- **No catalog manager for options.** Renaming an option, deleting one, or deleting a choice all rewrite every product using them, so they're deliberately not offered from a product page. The backend routes exist and 409 with the affected product list (`PUT|DELETE /api/options/:id`, `DELETE /api/options/:id/choices/:choiceId`) — they just have no UI. A `/options` page mirroring `frontend/src/pages/Logos.tsx` is the natural home.
